...
grid = lines.map { |l| l.split(' ').map{|s| s.to_i } }

width = grid.first.length
height = grid.length

candidates = []
candidates << updowns = (0...height-4).to_a.product((0...width).to_a).map{ |h, w| (0..3).map{|i| grid[h+i][w]}.inject(1, &:*) }.max
candidates << leftrights = (0...height).to_a.product((0...width-4).to_a).map{ |h, w| (0..3).map{|i| grid[h][w+i]}.inject(1, &:*) }.max
candidates << diagonals1 = (0...height-4).to_a.product((0...width-4).to_a).map{ |h, w| (0..3).map{|i| grid[h+i][w+i]}.inject(1, &:*) }.max
candidates << diagonals2 = (0...height-4).to_a.product((0...width-4).to_a).map{ |h, w| (0..3).map{|i| grid[h+i][w+3-i]}.inject(1, &:*) }.max
